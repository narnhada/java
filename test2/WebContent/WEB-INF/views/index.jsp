...
<title>메일링 리스트:목록</title>
	...
	<body>
		...
		<table border="1">
			...
			<tr>
				<td>이메일삭제</td>
				<td><a href="<%= request.getContextPath() %>/kim2?a=delete&no=<%=vo.getNo() %>">이메일 삭제</a></td>
			</tr>
			<tr>
				<td>이메일수정</td>
				<td><a href="<%= request.getContextPath() %>/kim2?a=update&no=<%=vo.getNo() %>">이메일 수정</a></td>
			</tr>
	</table>
		<% } %>
		<!-- 입력 폼으로 링크: 모델2 -->
		<a href="<%= request.getContextPath() %>/kim2?a=form">이메일 등록</a>	
	</body>
</html>